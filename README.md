...
**Назначение:** Получить список конечных рекламодателей.
...
- `name` (string): Наименование конечного рекламодателя (LIKE %name%).
...
**Назначение:** Создать нового рекламодателя.
...
- `update` (boolean): `true` – замена существующего элемента.
...
- `name` (string): Наименование контрагента (LIKE %name%).
- `uuid` (string): Уникальный идентификатор.
